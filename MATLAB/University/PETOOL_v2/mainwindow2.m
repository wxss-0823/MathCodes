...
function varargout = mainwindow2(varargin)
%MAINWINDOW2 M-file for mainwindow2.fig
%      MAINWINDOW2, by itself, creates a new MAINWINDOW2 or raises the existing
%      singleton*.
%
%      H = MAINWINDOW2 returns the handle to a new MAINWINDOW2 or the handle to
%      the existing singleton*.
%
%      MAINWINDOW2('Property','Value',...) creates a new MAINWINDOW2 using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to mainwindow2_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      MAINWINDOW2('CALLBACK') and MAINWINDOW2('CALLBACK',hObject,...) call the
%      local function named CALLBACK in MAINWINDOW2.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help mainwindow2

% Last Modified by GUIDE v2.5 08-Oct-2019 14:03:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @mainwindow2_OpeningFcn, ...
                   'gui_OutputFcn',  @mainwindow2_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before mainwindow2 is made visible.
function mainwindow2_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for mainwindow2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes mainwindow2 wait for user response (see UIRESUME)
...
set (findobj(handles.figure2,'tag','edit36'),'enable','inactive','backgroundcolor',[.83 .81 .78],'foregroundcolor',[0.5 0.5 0.5]);
set (findobj(handles.figure2,'tag','edit35'),'enable','inactive','backgroundcolor',[.83 .81 .78],'foregroundcolor',[0.5 0.5 0.5]);
set (findobj(handles.figure2,'tag','pushbutton11'),'enable','off','foregroundcolor',[0.5 0.5 0.5]); 
set (findobj(handles.figure2,'tag','plotbutton'),'enable','off','foregroundcolor',[0.5 0.5 0.5]); 
...
function varargout = mainwindow2_OutputFcn(hObject, eventdata, handles)
...
load flag
if runflag ~= 1
    set(findobj(handles.figure2,'tag','texterror'),'string','First click "Load Map" button to select DTED coordinates !');
    ...
    elseif (antennaheight <= 0)
        ...
        if exist('mapprofileparameter.mat') ~= 0
            load mapprofileparameter
        else
            return
        end
 
        
        Analysis_Parameter gui;
        uiwait;
        ...
        altitudepath = 'User_Files/terrain_3Dplot';
        mkdir([altitudepath '/' foldername])
        altitudedatapath = [altitudepath '/' foldername];
        save altitudepath altitudedatapath
                    
        resultpath = 'User_Files/output_result_3Dplot';
        mkdir([resultpath '/' foldername])
        finalpath = [resultpath '/' foldername];
        save resultpath finalpath

            
        warning_gui;      
        
        for angle=0:counter:endpoint
            a=distance(plat1,plon1,plat2,plon2);
            new_coor=reckon(plat1,plon1,a,degree+angle);
            plat2=new_coor(1);
            plon2=new_coor(2);
            ...
            [zz,rng,lat,lon] = mapprofile(Z,ref,plat,plon);
            
            %SECÝLEN KOORDÝNATLAR ARASI YÜKSEKLÝK DEÐÝÞÝMÝ
            if ~isempty(findobj('Name','Map'))
                figure(3);
                plot3m(lat,lon,zz,'r','LineWidth',2);
                hold on
            end
            ...
            maxheight = max(zz);
            heightval = str2num(get(findobj(handles.figure2,'tag','edit2'),'string'));
            if maxheight > heightval
                heightval = round(maxheight)+100;
                set (findobj(handles.figure2,'tag','edit2'),'string',num2str(heightval));
            end

            
            %% Save Altitude Data
            altitude.angle = degree+angle;
            altitude.range = b(:,1);
            altitude.elevation = b(:,2);
            
            if (degree+angle) == 360
                filename = 0;
            elseif degree+angle > 360
                filename = degree+angle - 360;
            else
                filename = degree + angle;
            end
            
            angle_list = [angle_list filename];
            angle_list_pol = [angle_list_pol degree+angle];
            savename = [altitudedatapath '/' num2str(filename) '.mat'];
            save(savename, 'altitude')
            
            %%
            ranger = num2str(b(:,1));
            xheighter = num2str(b(:,2));
            
            set (findobj(handles.figure2,'tag','listbox1'),'string','');
            set (findobj(handles.figure2,'tag','listbox2'),'string','');            
            set (findobj(handles.figure2,'tag','listbox1'),'string',ranger);
            set (findobj(handles.figure2,'tag','listbox2'),'string',xheighter);
            
%             latfile=num2str(plat2,3);
%             lonfile=num2str(plon2,3);
%             strname=[latfile '_' lonfile '.txt'];         
%             finalname = [fname '/' strname];
...
%             fclose(fid);
            ...
            if input_ready_flag
                ...
                if (x.deltaz < dzc) && (x.ground_type == 2)
                    ...
                    if (degree+angle) == 360
                        resultangle = 0;
                    elseif degree+angle > 360
                        resultangle = degree+angle - 360;
                    else
                        resultangle = degree + angle;
                    end
                    
                    result.angle = resultangle;
                    result.path_loss_final = path_loss_final;
                    result.prop_fact_final = prop_fact_final;
                    result.free_space_loss = free_space_loss;
                    result.range_vec = range_vec;
                    result.z_user = z_user;
                    result.z = z;
                    
                    savename = [finalpath '/' num2str(resultangle) '.mat'];
                    save(savename, 'result')
                end               
            end
        end
        
        save maxheight heightval
        save angle_list angle_list
        save angle_list_pol angle_list_pol
        close(warning_gui);
        
        set (findobj(handles.figure2,'tag','pushbutton11'),'enable','on','backgroundcolor',[.83 .81 .78],'foregroundcolor',[0 0 0]);
        set (findobj(handles.figure2,'tag','edit35'),'enable','on','backgroundcolor',[1 1 1],'foregroundcolor',[0 0 0]);
        set (findobj(handles.figure2,'tag','edit1'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit2'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu1'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu3'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit10'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit11'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit12'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit3'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton9'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton10'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu4'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit20'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit21'),'enable','on' );
        %         set (findobj(handles.figure2,'tag','radiobutton11'),'enable','on' );
        %         set (findobj(handles.figure2,'tag','radiobutton12'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton15'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton20'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit22'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu5'),'enable','on' );
        set (findobj(handles.figure2,'tag','listbox1'),'enable','on' );
        set (findobj(handles.figure2,'tag','listbox2'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton13'),'enable','on' );
        set (findobj(handles.figure2,'tag','radiobutton14'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu6'),'enable','on' );
        set (findobj(handles.figure2,'tag','popupmenu7'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit23'),'enable','on' );
        set (findobj(handles.figure2,'tag','listbox3'),'enable','on' );
        set (findobj(handles.figure2,'tag','listbox4'),'enable','on' );
        set (findobj(handles.figure2,'tag','listbox1'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton1'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton2'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton3'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton4'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton5'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton6'),'enable','on' );
        set (findobj(handles.figure2,'tag','edit24'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton7'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton8'),'enable','on' );
        set (findobj(handles.figure2,'tag','pushbutton9'),'enable','on');
        set (findobj(handles.figure2,'tag','plotbutton'),'enable','on','foregroundcolor',[1 1 1]);
        
        
        
        %% PLOT after execution
        load altitudepath
        load degree
        set (findobj(handles.figure2,'tag','edit35'),'string',num2str(degree));
        anglevalue = get(findobj(handles.figure2,'tag','edit35'),'string');
        set (findobj(handles.figure2,'tag','texterror'),'string','');
        load([altitudedatapath '/' anglevalue '.mat']);
        ranger = num2str(altitude.range);
        xheighter = num2str(altitude.elevation);
        ...
        if stopflag == 0
            ...
            load resultpath
            load([finalpath '/' num2str(anglevalue) '.mat'])
            
            path_loss_final = result.path_loss_final;
            prop_fact_final = result.prop_fact_final;
            free_space_loss = result.free_space_loss;
            range_vec = result.range_vec;
            z_user = result.z_user;
            z = result.z;
            ...
            set(handles.figure2,'WindowButtonMotionFcn','guimousevalue');
...
% if(fileName~=0)
%     l = strfind(fileName, '.txt');
%     m = strfind(fileName, '.dt0');
%     if(isempty(l) == 0)
...
range = []; angles = [];
anglevalue = str2num(get(findobj(handles.figure2,'tag','edit35'),'string'));

if (min(angle_list) > anglevalue || anglevalue > max(angle_list)) || ~ismember(anglevalue,angle_list)
    if angle_list(1) == angle_list(end)
        angle_list(end) = angle_list(end-1);
    end
    set (findobj(handles.figure2,'tag','texterror'),'string',['Angle should be between [',num2str((angle_list(1))), '-' ,num2str((angle_list(end))),'] with ',num2str(counter),' degree step']);
else
    if isempty(findobj('Name','Map'))
        warning_gui;
        load koordinatlar
    ...
    set (findobj(handles.figure2,'tag','texterror'),'string','');
    load([altitudedatapath '/' num2str(anglevalue) '.mat']);
    ranger = num2str(altitude.range);
    xheighter = num2str(altitude.elevation);
...
load analysisparameters
if (min(angle_list) > anglevalue || anglevalue > max(angle_list)) || ~ismember(anglevalue,angle_list)
    set (findobj(handles.figure2,'tag','texterror'),'string',['Angle should be between [',num2str(min(angle_list)), '-' ,num2str(max(angle_list)),'] with ',num2str(counter),' degree step']);
    return
end
load resultpath
load([finalpath '/' num2str(anglevalue) '.mat'])
...
default='No';
choice_save = questdlg(['Analysis Results for this angle already saved in User_Files/output_result_3Dplot folder with ',num2str(anglevalue) ,'.mat file name. Would you like to save it again?'],'Warning!','Yes','No',default);
switch choice_save
    case 'Yes'
        set(gcf,'WindowButtonMotionFcn','');
...
% LOAD MAP SET INITIAL POSITION
function pushbutton20_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton20 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
clc;
set(findobj(handles.figure2,'tag','edit35'),'string',' ');
set (findobj(handles.figure2,'tag','edit35'),'enable','inactive','backgroundcolor',[.83 .81 .78],'foregroundcolor',[0.5 0.5 0.5]);
set(handles.figure2,'WindowButtonMotionFcn','');
load inputfigs.mat;
% imt = image(figt,'Parent',handles.axestop);
% set(handles.axestop, 'Xtick',[], 'Ytick',[])
%figb = importdata('bottom.jpg');
...
if ~isempty(findobj('Name','3D Coverage Map'))
    close 'range_angle_plotanalysis';
end
set(findobj(handles.figure2,'tag','texterror'),'string',' ');
...
if dname ~= 0
    ...
    if isempty(latlim) || isempty (lonlim)
        ...
        Z(Z == 0) = -1;
        save mapprofileparameter Z ref
        ...
        if cancelflag == 0
            set(findobj(handles.figure2,'tag','edit36'),'string',num2str(start_degree));
            if ~isempty(range)
                range = range;
                angles = angles;
                new_coor=reckon(plat1,plon1,km2deg(range),angles);
...
%             offset_height = max(altitude, (maxheight * 1.2));
            ...
            set (findobj(handles.figure2,'tag','edit1'),'string',num2str((round((maxranger)))));
...
%             fclose(fid);
            ...
            %PLOT INITIAL Position Terrain Map
            axes(handles.axestop);
...
set (findobj(handles.figure2,'tag','radiobutton21'),'value',1);
